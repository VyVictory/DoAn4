...
const Profile = () => {
  ...
  const handleScroll = useCallback(() => {
    if (targetRef.current && height != 0) {
      ...
      if (rect.bottom - window.innerHeight - 3 < 0) {
        setIsPassed(rect.bottom - window.innerHeight - 3 < 0);
      } else if (rect.bottom - window.innerHeight + 2 > 0) {
        setIsPassed(rect.bottom - window.innerHeight + 2 < 0);
      }
    }
  }, [height]);
  ...
  return (
    <div className="NavbarUser flex overflow-auto">
      <div className="w-full h-full flex items-center flex-col">
        <div className="w-full profileW  z-10 px-2">
        ...
        <div className="h-4 w-full mb-4 shadow-xl"></div>
        ...
        <div className=" flex  w-full justify-center">
          ...
          <div
            ...
            className={`${
              isPassed
                ...
                : ""
            }  min-h-screen space-y-4 profileW`}
          >
            <div
              className="w-2/5 pr-4 pb-4 space-y-4 hidden md:block"
              style={{
                position: "relative",
                zIndex: "20",
              }}
            >
              {[...Array(6)].map((_, index) => (
                <div
                  className="border border-gray-200 rounded-lg Post p-2 ShadowContent"
                  key={index}
                >
          ...
          {/* right */}
          <div className="absolute w-full flex profileW justify-center">
            <div className=" x flex flex-row justify-center">
              {/* // độn div left */}
              <div className="  min-h-screen w-2/5 hidden md:block"></div>
              <div className="flex flex-col space-y-4  md:w-3/5 px-6 md:px-0">
                {[...Array(20)].map((_, index) => (
                  <div
                    className="border border-gray-200 rounded-lg Post p-2 w-full ShadowContent"
                    key={index}
                  >
                    HÌNH ẢNH VÀ VIDEO BẠN ĐANG XEM LÀ 100% TỪ GAME Tặng VIPcode:
                    VIP666 l DGDL999 l RONGTHAN Webgame chơi Online ngay trên PC
