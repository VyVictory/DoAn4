...
export default function Register({ chaneform }) {
    ...
    const validateForm = () => {
        ...
        if (!formData.birthDate) validationErrors.birthDate = 'Vui lòng nhập ngày sinh.';
    ...
    const handleSubmit = async () => {
        ...
        if (Object.keys(validationErrors).length === 0) {
            try {
                ...
                if (response) {
                    toast.success('Đăng ký thành công! Bạn có thể đăng nhập ngay.', NotificationCss.Success);
                }
            } catch (error) {
                console.error('Lỗi:', error);
                toast.error('Đăng ký thất bại, vui lòng thử lại.', NotificationCss.Fail);
            }
        } else {
    ...
    return (
        <div className="flex justify-center items-center h-screen w-full bg-transparent">
            <div
                className="bg-white shadow-lg shadow-gray-500 rounded-2xl w-full max-w-lg my-4 flex flex-col justify-between overflow-y-auto max-h-[90vh]"
            >
                <button
                ...
                <div className="p-10">
                    ...
                    <div className="grid sm:grid-cols-1 md:grid-cols-2 gap-4 mb-5">
                        <div className="col-span-1">
                            <label htmlFor="birthDate" className="block text-gray-600 text-sm font-medium">
                                Ngày sinh
                            </label>
                            <input
                                type="date"
                        ...
                        <div className="col-span-1">
                            <label htmlFor="gender" className="block text-gray-600 text-sm font-medium">
                                Giới tính
                            </label>
                            <select
                                name="gender"
                    ...
                    <div className="flex items-center justify-between mt-3 mb-4 text-nowrap">
                        <span className="text-sm text-gray-400">Đã có tài khoản?</span>  {/* Change Link to span */}
                        <button
